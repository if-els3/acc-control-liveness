...
def menu_daftar_pengguna(db: Database, face_engine: FaceEngine):
    """Daftarkan pengguna baru: RFID + wajah."""
    ...
    if not cam.start():
        ...
        if input("[y/N] : ").strip().lower() == 'y':
            user_id = db.tambah_user(nama, uid_str, embeddings=None)
    ...
    if len(embeddings) == 0:
        ...
        print("      Simpan data tanpa wajah? ", end="")
        if input("[y/N] : ").strip().lower() == 'y':
            db.tambah_user(nama, uid_str, embeddings=None)
    ...
    user_id = db.tambah_user(nama, uid_str, embeddings=embeddings)
